...
public:
    ...
    pair<int, int> transform(int doc_idx){
...
int ** compat(int ** SignatureMatrix, int r){
    int tmp_SignatureMatrix[SIGNATURES_NUM % r][11] = {0}
    for(int i = 0; i < (SIGNATURES_NUM % r); i++){
        for(int j = 0; j < DOCS_NUM; j++){
            int tmp = 0;
            tmp_SignatureMatrix[i][j] = 100 * SignatureMatrix[i * r][j] + 10 * SignatureMatrix[i * r + 1][j] + SignatureMatrix[i * r + 2][j];
        }
    }
}
